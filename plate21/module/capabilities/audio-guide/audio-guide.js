...
Component({
  ...
  methods: {
    ...
    onToggleDeep() {
      ...
      this.setData({ deepOpen: next, guest: null })
      if (next) {
        const key = 'deepOpened_' + this.data.station
        const snap = sessionStore.getSnapshot()
        if (!snap || !snap.flags[key]) sessionStore.setFlag(key, Date.now()).catch(() => {})
      }
    },
    onToggleDeepPlay() {
      if (this.data.deepPlaying) audio.get().pause()
      else if (this.data.guide && this.data.guide.deepAudio)
        audio
          .get()
          .playVoice({
            id: 'deep-' + this.data.station,
            title: this.data.guide.title + ' · 深讲',
            src: this.data.guide.deepAudio
          })
    },
    onFab() {
