/**
 * 语音导览文稿配置 —— 每站两层（v2 双版本机制）：
 *   script    基础层（约1分钟）：方向 + 一个知识锚 + 生疑句，到站默认展开；
 *   deepScript 深讲层（约2-3分钟）：史实细节 + 争议口径 + 档案主人线索，
 *             永远折叠、玩家主动展开，绝不自动播。
 * audio / deepAudio 暂为 null（素材待录制）：届时只回填路径，组件与页面零改动。
 * guestPostcard：该站深讲层展示「一位考察者留下的明信片」（本地种子池降级，
 * 后端就绪后换真实留言池单张）。信息点依据 docs/方案设计-…-v2对齐.md §3.2，
 * 口吻待剧情整体定稿后统一润色，本稿只固化信息。
 */
'use strict'

const GUIDES = {
  s2: {
    station: 's2',
    title: '黄花阵 · 语音导览',
    audio: null,
    script: [
      '你眼前的这座迷宫叫黄花阵，仿照欧洲迷宫而建，却是地道的皇家游园。',
      '阵墙以青砖砌成，不过一人高，墙面满刻万字不断纹，取“福寿绵长”之意。',
      '中秋之夜，宫女手持黄色彩绸扎成的莲花灯穿行阵中，皇帝登上中央圆亭，看谁的灯最先到亭前。'
    ],
    deepAudio: null,
    deepScript: [
      '档案里夹着一张1987年的工地照片。照片背面一行钢笔字：照原图，复位。',
      '这道墙不是乾隆年间的原物——1987年和1989年，人们照着两百年前的铜版画，把阵墙和中心圆亭在原址按原样重新砌了起来。他们手上那幅画，就是我一直在找的那套铜版画里的一张。',
      '换句话说：早就有人做过一次「照着一幅画，把地上的东西补回来」的事——而且做成了。',
      '官网记载，阵墙总长一千六百余延长米，墙高约一点二米，墙面镶卍字雕花青砖。你刚才穿过的，就是那幅画重新落地的样子。',
      '这件事先记下，不急着下结论。走到最后你会再想起这道墙。'
    ]
  },
  s3: {
    station: 's3',
    title: '海晏堂 · 大水法 · 语音导览',
    audio: null,
    script: [
      '海晏堂是西洋楼诸景中规模最大的建筑，阶前这座喷水池，曾是一座以水报时的钟。',
      '池边十二生肖铜像各代表一个时辰，到点由对应的兽首喷水；正午时分，十二首齐喷。',
      '如今鼠、牛、虎、兔、马、猴、猪七尊已经归来，其余五尊仍下落不明。'
    ],
    deepAudio: null,
    deepScript: [
      '档案翻到水力钟的图纸页。给十二尊兽首定时刻的机械，也是从欧洲来的——设计它的人叫蒋友仁。',
      '十二兽首里，已有七尊归来：牛、虎、猴、猪、鼠、兔、马。其余五尊，至今下落不明。',
      '马首由何鸿燊先生出资购回，2019年捐赠国家文物局，2020年12月回到圆明园，在正觉寺文殊亭展出——它是第一件回归原址的兽首。',
      '这座钟两百年后还在走——只是报时的东西，从水变成了回归的消息。',
      '这么多喷泉同时喷水，却没有电泵。水从哪儿来？答案在海晏堂的背面。',
      '等你绕到那座高高的土台底下，记得抬头看看——下一个问题就在那儿等着。'
    ]
  },
  s4: {
    station: 's4',
    title: '雨果雕像 · 语音导览',
    audio: null,
    script: [
      '档案里离得最远的一手，来自一个从没来过中国的人。1860年圆明园罹劫；次年，他写下《致巴特勒上尉的信》。',
      '他称圆明园为“世界奇迹”，公开谴责这场掠夺：“两个强盗闯进了圆明园。一个叫法兰西，一个叫英吉利。”',
      '2010年，罹难150周年之际，这尊雕像在此落成，面朝大水法残柱。'
    ],
    deepAudio: null,
    guestPostcard: true,
    deepScript: [
      '雨果没有来过中国。他关于圆明园的一切，来自掠夺品清单、随军画家的速写和报纸上的文章——他想象出来的圆明园，比亲眼见过的更接近真相。',
      '“两个强盗”是文学修辞，不是史实记录。他用的是文学手法，不是报道——这一点值得知道。',
      '1861年11月25日，他写下那封信；2010年，法国雕塑家娜什拉·凯努无偿创作了这尊雕像，面朝它凝视的废墟。',
      '留一个问题给你，没有标准答案：一样东西放在哪儿，才算被保住了？'
    ]
  },

  /* ===== v2 顺路支线站（挂 waypoint/dashuifa 页，经 audio-station 传入） ===== */

  't-xieqiqu': {
    station: 't-xieqiqu',
    title: '顺路 · 谐奇趣 · 语音导览',
    audio: null,
    script: [
      '去黄花阵的路上会先撞见一片汉白玉残基——谐奇趣，乾隆十六年秋天建成，是西洋楼里第一座盖起来的欧式水法大殿。',
      '主楼三层，楼前两侧各有九间弧形游廊，连着两座八角楼厅。',
      '档案上写着：乾隆在主楼进膳，两侧的琉璃厅一边奏东方的乐器，一边奏西方的，楼前看喷泉。他就坐在正中间。'
    ],
    deepAudio: null,
    deepScript: [
      '这地方不是欧洲，也不是中国。它是一个中国皇帝按自己的趣味，把两边都搬过来，摆在左右手边的样子。整个西洋楼是什么性质，站在这儿想一遍，比看多少资料都清楚。',
      '南边那座海棠形的水池里，原来有铜羊、铜鸭，还有一条西洋式的翻尾石鱼，一起喷水。我在池底找了一圈——基座还在，鱼没有。',
      '后来我查到了它在哪儿：那条石鱼现在在北京大学的未名湖里，还在水里。先记下一句话——东西不在原来的地方，不一定就是没了。这句话后面还会想起好几次。'
    ]
  },

  't-yangquelong': {
    station: 't-yangquelong',
    title: '顺路 · 养雀笼 · 语音导览',
    audio: null,
    script: [
      '往海晏堂走，右手边会先出现一座几乎完整的大理石门框——养雀笼。中间是过道门，两侧原来养着孔雀和各种珍禽，从这道门穿过去，就进入西洋楼的东半部。',
      '东面是大理石西洋门：券顶、壁柱、雕花，纹样是西洋的涡卷。',
      '西面原是中式木牌坊。木头的那一半，1860年烧了。留下来的那一半，是石头做的。'
    ],
    deepAudio: null,
    deepScript: [
      '如果道具包里有那两张叠层卡：一张印东面石门的线稿，一张印西面牌坊的复原线稿。叠起来对着天光看——两个半边合成一座完整的门。',
      '放下纸，眼前只有石头的那一半。',
      '同一座建筑，一半留下来了，一半没有。留下来的那一半，是石头做的。'
    ]
  },

  't-fangwaiguan': {
    station: 't-fangwaiguan',
    title: '顺路 · 方外观 · 语音导览',
    audio: null,
    script: [
      '再往前几十步，是一座三开间的台基——方外观，乾隆二十四年建成，原来是一座礼拜殿，供着阿拉伯文碑刻。',
      '档案说，这里是给容妃做礼拜用的。她确有其人；「香妃」「体有异香」是民间传说，没有实证。',
      '正对面隔着石桥是五竹亭：五座西洋式重檐亭子，连着十八间竹廊。档案说，她每次来，乾隆都陪着来。但他不进去。'
    ],
    deepAudio: null,
    deepScript: [
      '他是皇帝，想进当然能进去。他没进。档案上就一句话，没写他等了多久，也没写他在想什么。',
      '纹样对照卡：绕着台基走一圈，能找到几种纹样就划几种。档案上的数目和现场对不上——对不上就对上了，我不做裁判。',
      '如果道具包里有那张等候卡——想到了一个人，就把名字写在背面。写完就收起来吧，这张不用给我看。',
      '这一站留给你的问题是：能留下来的，到底是什么。'
    ]
  },

  't-xushuilou': {
    station: 't-xushuilou',
    title: '顺路 · 蓄水楼 · 语音导览',
    audio: null,
    script: [
      '绕到海晏堂背面，是一座很高的工字形土台——蓄水楼，也叫锡海。楼顶是铺了锡板防渗的大蓄水池，一次能蓄一百六十多立方米水。',
      '水提到楼顶，靠高差的压力经铜管送到各处喷口——不用任何动力，因为水已经在高处了。',
      '那么只剩一个问题：谁把水提上去的？举起量高绳比一比这个台子有多高——水要被提到那个高度，一天十二个时辰，不能停。'
    ],
    deepAudio: null,
    deepScript: [
      '关于那套提水的水车到底怎么转，我的档案这一栏是空着的。三种说法都摆给你，我一种也不敢选——',
      '一说人力蹬攀，见于《圆明园研究》第86期；一说畜力齿轮，郭黛姮团队据蒋友仁书信复原；还有一份图说写的是骡子拉水车。到今天还在争。',
      '设计这套水车的是法国人蒋友仁，乾隆三十九年在北京去世。石头还在，土台还在，没了的是那套让水上去的办法——连它当年怎么转的，我们都不确定了。'
    ]
  },

  't-xianfahua': {
    station: 't-xianfahua',
    title: '顺路 · 线法画 · 语音导览',
    audio: null,
    script: [
      '几排砖墙的基址，一层层往里收，中间一条笔直的通道——线法画。',
      '乾隆命人用西洋的透视法，把西洋风景画画在墙上：从方河西岸往东看，七道断墙会显出纵深，像一条望不到头的西方街市。',
      '如今线法墙已荡然无存，方河已清整植荷。'
    ],
    deepAudio: null,
    deepScript: [
      '《圆明园研究》第86期据样式雷图档指出：两列十一面梯形收口，强化了一点透视的效果——引用请点名86期。',
      '又想起方外观那位维吾尔族的妃子。她的家乡在很远的西边，他没法把她送回家——就在园子最东头，用透视法造了一条假的欧洲街。',
      '如果你手里有那张雪山线稿：举起来，纸上的雪山会落回空墙上。放下纸，又没有了。'
    ]
  }
}

module.exports = { GUIDES }
