...
test('listening preference gates auto narration and only current-page audio can begin countdown', async () => {
  ...
  h.page.onNarrationEnded({ detail: { contextKey: 'old-page' } })
  assert.equal(h.page.data.autoSeconds, 0)
  h.page.onNarrationEnded({ detail: { contextKey: h.page.data.narrationKey } })
  assert.equal(h.page.data.autoSeconds, 3)
  h.page.onUserInteraction(); assert.equal(h.page.data.autoSeconds, 0)
  h.page.onNarrationEnded({ detail: { contextKey: h.page.data.narrationKey } })
  await h.invoke('onDrawer', event({ name: 'history' }))
  assert.equal(h.page.data.autoSeconds, 0)
  ...
  h.page.onNarrationEnded({ detail: { contextKey: h.page.data.narrationKey } })
  assert.equal(h.page.data.autoSeconds, 0)
  await h.invoke('onChooseMode', event({ mode: 'read' }))
  assert.equal(h.page.data.listenKey, '')
  await h.close()
})
