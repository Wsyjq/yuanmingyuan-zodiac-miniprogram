...
- 手账阅读器 `novel-view` 密度化分页（`utils/novel-pages.js`）：按字数权重把段落打包成整页（目标约 180 字/页，图片折算 70），首字下沉只保留在每页首个叙述块。序章由 10 页收敛到约 4 页，终章幕 4 由 9 页收敛到约 3 页；剧情文本未改动。
...
2026-08-19 增量复验（能力系统 + 成就系统 + 阅读器密度化合入后）：

| 门禁 | 结果 |
|---|---|
| 静态检查 | `syntax=91 json=43 routes=17`（worktree 干净检出下为 `85/36/17`，差异来自主树本地忽略目录） |
| Node 测试 | 106 项通过，0 失败（新增 novel-pages 7 项，novel-view 组件契约随多块分页更新） |
